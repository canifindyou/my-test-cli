    ...
    <div v-if="data !== ''">
        <div class="container">
            <div class="row" style="padding:5px;display: inline-flex;width:100%">
                <div class="m-nav">
                    <ul class="m-nav-ul">
                        <li class="title">专业活动</li>
                        <ul class="m-nav ac-ul" v-if="data !== ''" v-for="(item,index) in data.nav">
                            <li :class="{'ac-active' : isSelect === index}" @click="clickItem(index,item.flag)">{{item.title}}</li>
                        </ul>
                    </ul>
                </div>
...
export default {
    ...
    mounted() {
        ...
        this.data = this.readFile("../../../static/file/majorActive/majorActive.json")
    },
    methods: {
        //面包屑导航
        initRouter() {
        ...
        goto(path, num) {
            this.$router.push({
                path: path,
                query: { code: this.$route.query.code,listNum:this.$route.query.listNum }
            });
        }
    }
};
